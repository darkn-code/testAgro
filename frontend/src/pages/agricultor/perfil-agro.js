...
function Perfil() {
  ...
  useEffect(() => {
    if (!username) return;
    fetch(`http://192.168.1.70:5000/api/usuario/${username}`)
      .then(res => res.json())
      .then(data => {
        if (!data.error) {
          setUsuario(data);
  ...
  const subirFoto = (archivo) => {
    ...
    fetch(`http://192.168.1.70:5000/api/usuario/${username}/foto`, {
      method: 'POST',
      body: formData
    })
      .then(res => res.json())
    ...
    <div className="principal-perfil">
      <div className="contenedor-perfil">
        <form className="datos-usuario" onSubmit={actualizarDatos}>
          <h2>Perfil</h2>
          {usuario.ruta_foto && (<img className="usuario-foto" src={`http://192.168.1.70:5000${usuario.ruta_foto}`} alt="foto-perfil" onClick={() => document.getElementById("archivoImagen").click()} style={{ cursor: "pointer" }}/>)}
